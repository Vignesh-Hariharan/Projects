# Data Projects

Portfolio of data projects demonstrating end-to-end pipeline development, analytics engineering, and data architecture.

## Projects

### [Project 1: Multi-Touch Attribution Engine](./attribution-analytics/)
Attribution analytics pipeline comparing credit allocation across models, highlighting how prospecting channels are consistently undervalued with traditional last-click analytics.

**Stack:** Python, Snowflake, dbt, Tableau   
**Skills Used:** Attribution modeling, dimensional modeling, advanced SQL (window functions), dbt transformations, Python ETL, Snowflake data warehousing, orchestration (bash/CLI), synthetic data generation

---

### Project 2

end-to-end fraud detection pipeline using Snowflake, dbt, and Python. Loaded 1.3M transactions, engineered 15 features with dbt transformations, and automated predictions using Snowflake Cortex ML. Ran 4 experiments and found simpler models (6 features) outperformed complex ones (18 features).

**Stack:** Python, Snowflake Cortex, dbt, Tableau, Slack alerts

---

## Tech Stack

**Cloud & Warehousing:** Snowflake, AWS S3, BigQuery  
**Transformation:** dbt, SQL, Python  
**Orchestration:** Kestra, Airflow  
**Visualization:** Tableau, Streamlit, ThoughtSpot  
**ML/AI:** Snowflake Cortex, OpenAI API  
**Required for you:** Snowflake and AWS access, Python, dbt, Tableau or Streamlit, Kestra or Airflow account, OpenAI API key
